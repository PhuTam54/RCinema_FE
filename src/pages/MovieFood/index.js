...
function MovieFood() {
  ...
  useEffect(() => {
    ...
    axios.get(`https://localhost:7168/api/v1/Foods`)
      .then((response) => {
  ...
  const handleCheckout = () => {
    const orderData = {
      ...
      'final_Total': totalPrice + bill.reduce((acc, curr) => acc + (curr.price * curr.qty), 0) - 1, // Giả sử có giảm giá 1 đơn vị
      'status': "Chua Thanh Toán",
      'payment_Method': 'vnpay',
      'qR_Code': 'thisIsAQRCode',
      'foods': bill.map(item => ({ name: item.name, qty: item.qty, price: item.price })), // Thêm thông tin về đồ ăn vào đơn hàng
                      ...
                      <div className="grid-inner">
                        ...
                        <div className="grid-content">
                          ...
                          <form className="cart-button">
                            ...
                            <button type="button" className="custom-button" onClick={() => addToBill(food, parseInt(document.getElementsByName('qty')[index].value))}>
                              add
                            </button>
                          </form>
                        </div>
            ...
            <div className="col-lg-4">
              <div className="booking-summery bg-one">
                <div className="section-header-3" style={{ margin: 0 }}>
                  <h6 style={{ textAlign: "center" }} className="cate">Room {roomId}</h6>
                </div>
                ...
                <li>
                  ...
                  <li>
                    <h6 className="subtitle mb-0">
                      ...
                      <span>${totalPrice + bill.reduce((acc, curr) => acc + (curr.price * curr.qty), 0)}</span>
                    </h6>
                  </li>
                </ul>
              </div>
              <div className="proceed-area  text-center">
                ...
                <Link onClick={handleCheckout} className="custom-button" to={`/moviecheckout/${movies.id}/show/${showId}`}>
                  Seat Plans
